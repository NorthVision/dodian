			...
			if(playersCurrentlyOn[i] != null)
 {
	...
	public void process() {
		...
		try{
			...
			cycle++;
			/*for(String h : server.connections){
				System.out.println("Removing connection " + server.connections.get(0));
			...
			/*for(int mId = 0; mId < matches.length; mId++){
				if(matches[mId] == null) continue;
				if(matches[mId].gameOver) matches[mId] = null;
				...
				try{
					...
					if(!players[i].disconnected && !players[i].isActive){
						if(players[i].violations > 20){
							System.out.println("Disconnecting bugged player " + players[i].playerName);
					...
					if(players[i].disconnected) continue;
					if(players[i].dbId < 1 && System.currentTimeMillis() - players[i].lastPacket >= 15000){
						misc.println("Disconnecting possible null " + players[i].playerName);
					...
					if(!players[i].playerName.equals("null"))
			...
			for(int i = 0; i < maxPlayers; i++) {
				if(players[i] == null) continue;
				if(!players[i].isActive || players[i].playerName == null || players[i].playerName.equals("null")) continue;
				...
				if(lp > 15000){
					System.out.println("Removing non-responding player " + players[i].playerName);
					players[i].disconnected = true;
				}
				if(players[i].disconnected) {
	...
	public void removePlayer(Player plr) {
		...
		if(plr.Privatechat != 2) { //PM System
			for(int i = 1; i < maxPlayers; i++) {
				if (players[i] == null || players[i].isActive == false || players[i].playerName == null || players[i].playerName.equals("null")) continue;
